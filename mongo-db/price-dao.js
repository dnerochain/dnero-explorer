...
module.exports = class priceDAO {
  ...
  async getPrice(callback) {
    ...
    if (this.redis !== null) {
      let list = [];
      try {
        list = await this.redis.hmget(redis_key, 'DNERO', 'DTOKEN');
      } catch (e) {
    ...
    function query() {
      const queryObject = { '_id': { $in: ['DNERO', 'DTOKEN', 'DDROP'] } };
      self.client.query(self.priceInfoCollection, queryObject, function (error, recordList) {
        if (error) {
          ...
          if (self.redis !== null) {
            self.redis.hmset(redis_key, 'DNERO', JSON.stringify(recordList[0]), 'DTOKEN', JSON.stringify(recordList[1]));
          }

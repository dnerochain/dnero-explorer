...
var blockRouter = (app, blockDao, progressDao, checkpointDao, config) => {
  ...
  //             total += (tx.data.source.coins.dtokenwei - '0') / 1000000000000000000;
  //           } catch (e) {
    ...
    if (blockId > 0) {
      progressDao.getProgressAsync(config.blockchain.networkId)
        ...
        .then(async blockInfo => {
          if (blockInfo.height % 100 === 1 && blockInfo.sentry_votes) {
            try {
              let checkpoint = await checkpointDao.getCheckpointByHashAsync(blockInfo.sentry_votes.Block);
              let voted = new BigNumber(0), deposited = new BigNumber(0), j = 0;
              for (let i = 0; i < checkpoint.sentrys.length; i++) {
                let skip = true;
                checkpoint.sentrys[i].Stakes.forEach(stake => {
                  skip = skip && stake.withdrawn;
                  let dnero = !stake.withdrawn ? stake.amount : new BigNumber(0);
                  let multi = !blockInfo.sentry_votes.Multiplies[j] ? 0 : 1
                  voted = voted.plus(multi ? dnero : 0);
                  deposited = deposited.plus(dnero)
                })
                j += skip ? 0 : 1;
              }
              blockInfo.total_deposited_sentry_stakes = deposited;
              blockInfo.total_voted_sentry_stakes = voted;
            } catch (err) {
              console.log(err)
            };
          }
          delete blockInfo.sentry_votes;
          const data = ({
            type: 'block',
            body: blockInfo,

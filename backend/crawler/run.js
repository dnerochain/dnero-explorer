...
function main() {
  ...
  if (!config.defaultDneroChainID) {
    Logger.log('Error: unable to load config.defaultDneroChainID:', config.defaultDneroChainID);
    process.exit(1);
  }
  Dnero.chainId = config.defaultDneroChainID;
  Logger.log('Dnero.chainId:', Dnero.chainId);
...
function setupGetBlockCronJob(mongoClient, networkId) {
  ...
  accountingJob.InitializeForDTokenPrice(accountingDao, config.accounting.coinmarketcapApiKey, config.accounting.walletAddresses);
  schedule.scheduleJob('Record DToken Price', '0 0 0 * * *', 'Etc/GMT', accountingJob.RecordDTokenPrice); // GMT mid-night

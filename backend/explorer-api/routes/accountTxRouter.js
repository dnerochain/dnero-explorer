          ...
          switch (tx.type) {
              ...
              if (data.proposer.address !== address) {
                data.outputs.forEach(output => {
                  if (output.address === address) {
                    obj.tx_type = 'Receive';
                    obj.dnero_amount = helper.formatCoin(output.coins.dnerowei);
                    obj.dtoken_amount = helper.formatCoin(output.coins.dtokenwei);
                    obj.from = '0x00000';
                    obj.to = address;
                  }
                })
            ...
            case 2:
              if (data.inputs[0].address === address) {
                obj.tx_type = 'Send';
                obj.dnero_amount = helper.formatCoin(data.inputs[0].coins.dnerowei);
                obj.dtoken_amount = helper.formatCoin(data.inputs[0].coins.dtokenwei);
                obj.from = address;
            ...
            case 5:
              if (data.source.address === address) {
                obj.tx_type = 'Send';
                obj.dnero_amount = helper.formatCoin(data.source.coins.dnerowei);
                obj.dtoken_amount = helper.formatCoin(data.source.coins.dtokenwei);
                obj.from = address;

...
export default class Stakes extends React.Component {
  ...
  render() {
    ...
    const title = `TOTAL ${stakeCoinType === 'dtoken' ? 'DTOKEN' : 'DNERO'} STAKED`;
    const legend = stakeCoinType === 'dtoken' ? 'DTOKEN NODES' : 'DNERO NODES';
    return (
      <div className="content stakes">
        <div className="page-title stakes">{title}</div>
        <div className="chart-container">
          <DneroChart chartType={'doughnut'} labels={holders} data={percentage} clickType={'account'} />
        </div>
        <div className="legend">
          {legend}
          <div className="stake-switch-container">
            STAKED TOKEN
            <Link to={stakeCoinType === 'dnero' ? "/stakes/dtoken" : "/stakes/dnero"}>
              <label className="stake-switch">
                <input type="checkbox" className="checkbox" defaultChecked={stakeCoinType === 'dtoken'} />

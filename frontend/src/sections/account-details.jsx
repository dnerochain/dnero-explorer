...
export default class AccountDetails extends React.Component {
  ...
  setStakesTNS = async (dneroSourceTxs, dtokenSourceTxs, dneroHolderTxs, dtokenHolderTxs) => {
    const uniqueAddresses = arrayUnique(
    ...
    const domainNames = await tns.getDomainNames(uniqueAddresses);
    dneroSourceTxs.map((x) => { x.toTns = x.holder ? domainNames[x.holder] : null });
    dtokenSourceTxs.map((x) => { x.toTns = x.holder ? domainNames[x.holder] : null });
    dneroHolderTxs.map((x) => { x.toTns = x.source ? domainNames[x.source] : null });
    dtokenHolderTxs.map((x) => { x.toTns = x.source ? domainNames[x.source] : null });
    this.setState({
      dneroHolderTxs,
      dneroSourceTxs,
      dtokenHolderTxs,
  ...
  getStakeTransactions(address) {
    ...
    stakeService.getStakeByAddress(address)
      .then(res => {
        ...
        this.setStakesTNS(dneroSourceTxs, dtokenSourceTxs, dneroHolderTxs, dtokenHolderTxs);
      })
      .catch(err => {
        console.log(err);
      });

...
export default class BlocksExplorer extends React.PureComponent {
  ...
  render() {
    ...
    const isCheckPoint = block && (block.total_voted_sentry_stakes != undefined);
    const dtokenBurnt = block ? totalTfuelBurnt(block) : new BigNumber(0);
    return (
          ...
          <React.Fragment>
            <table className="txn-info details">
              <tbody className={cx({ 'cp': isCheckPoint })}>
                ...
                <tr>
                  <th>Tfuel Burnt</th>
                  <td><div className="currency dtokenwei">{dtokenBurnt.toString(10)}</div></td>
                </tr>
                {isCheckPoint && <tr>
                  <th className="cp"># Voted Sentry Stakes</th>
                  <td>
                    <div className="currency dnerowei left">{formatCoin(block.total_voted_sentry_stakes)} Dnero</div>
                    <div className='price'>&nbsp;{`[\$${priceCoin(block.total_voted_sentry_stakes, price['Dnero'])} USD]`}</div>
                  </td>
                </tr>}
                {isCheckPoint && <tr>
                  <th className="cp"># Deposited Sentry Stakes</th>
                  <td>
                    <div className="currency dnerowei left">{formatCoin(block.total_deposited_sentry_stakes)} Dnero</div>
                    <div className='price'>&nbsp;{`[\$${priceCoin(block.total_deposited_sentry_stakes, price['Dnero'])} USD]`}</div>
                  </td>
                </tr>}
                <tr>
                  <th>Proposer</th>

...
export function totalCoinValue(set, key = 'dtokenwei') {
...
export function fee(txn) {
  let f = get(txn, 'data.fee.dtokenwei');
...
export function getTfuelBurnt(txn) {
  ...
  if (txn.type === 7) {
    ...
    dtokenwei = timeCoin(gasUsed, gasPrice);
  } else {
    dtokenwei = txn.data && txn.data.fee ? txn.data.fee.dtokenwei : 0;
  }
  return BigNumber(dtokenwei).dividedBy(WEI).toString();
}

export function value(txn) {
  let values = [
    totalCoinValue(get(txn, 'data.inputs'), 'dtokenwei'),
    totalCoinValue(get(txn, 'data.inputs'), 'dnerowei')];
  return chain(values)
...
export function coins(txn, account = null) {
  let coins = { 'dnerowei': 0, 'dtokenwei': 0 };
  ...
  switch (txn.type) {
    case TxnTypes.COINBASE:
      ...
      if (!account || txn.data.proposer.address === account.address) {
        coins = {
          'dnerowei': totalCoinValue(get(txn, 'data.outputs'), 'dnerowei').toFixed(),
          'dtokenwei': totalCoinValue(get(txn, 'data.outputs'), 'dtokenwei').toFixed()
        }
    ...
    case TxnTypes.TRANSFER:
      ...
      if (!account) {
        coins = {
          'dnerowei': totalCoinValue(get(txn, 'data.inputs'), 'dnerowei').toFixed(),
          'dtokenwei': totalCoinValue(get(txn, 'data.inputs'), 'dtokenwei').toFixed()
        }

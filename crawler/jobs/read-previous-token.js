...
async function updateTokens(txs, smartContractDao, tokenDao, tokenSummaryDao) {
  ...
  for (let tx of txs) {
    ...
    // Only log of DTokenVoucherMinted event remains
    logs = logs.map(obj => {
      ...
      switch (get(log, 'topics[0]')) {
        ...
        case EventHashMap.DTOKEN_VOUCHER_MINTED:
          if (typeof get(log, 'decode') !== "object") {
            ...
            insertList.push(checkAndInsertToken(xDtokenInfo, tokenDao))
          }
          break;
...
function _getOtherContractAddressSet(tx) {
  ...
  logs.forEach(log => {
    ...
    if (get(log, 'topics[0]') === EventHashMap.TRANSFER || get(log, 'topics[0]') === EventHashMap.DTOKEN_VOUCHER_MINTED
      || get(log, 'topics[0]') === EventHashMap.TRANSFER_SINGLE || get(log, 'topics[0]') === EventHashMap.DTOKEN_VOUCHER_BURNED) {

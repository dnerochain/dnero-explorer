...
var Dnero = require('../libs/Dnero');
var DneroJS = require('../libs/dnerojs.esm');
...
exports.updateToken = async function (tx, smartContractDao, tokenDao, tokenSummaryDao, tokenHolderDao) {
  ...
  for (let [i, log] of logs.entries()) {
    switch (get(log, 'topics[0]')) {
      case EventHashMap.DTOKEN_SPLIT:
        if (typeof get(log, 'decode') !== "object") {
          log = decodeLogByAbiHash(log, EventHashMap.DTOKEN_SPLIT);
          let sellerInfo = {
      ...
      case EventHashMap.TRANSFER:
        ...
        if (contractList.indexOf(contractAddress) > -1) {
          let type = '';
          if (contractMap.DNC20TokenBank.indexOf(contractAddress) !== -1) {
            type = 'XCHAIN_DNC20'
          } else if (contractMap.DNC721TokenBank.indexOf(contractAddress) !== -1) {
            type = 'XCHAIN_DNC721'
          }
...
exports.updateTokenByTxs = async function (txs, smartContractDao, tokenDao, tokenSummaryDao,
  ...
  for (let tx of txs) {
    ...
    for (let [i, log] of logs.entries()) {
      const contractAddress = get(log, 'address');
      console.log('chainType:', chainType, '.contractMap.DTokenTokenBank:', contractMap.DTokenTokenBank);
      switch (get(log, 'topics[0]')) {
        case EventHashMap.DTOKEN_SPLIT:
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DTOKEN_SPLIT);
        ...
        case EventHashMap.TRANSFER:
          ...
          insertList.push(_checkAndInsertToken(newToken, tokenDao))
          break;
        case EventHashMap.DTOKEN_VOUCHER_MINTED:
          if (contractMap.DTokenTokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DTOKEN_VOUCHER_MINTED);
            Logger.log('Decoded DTOKEN_VOUCHER_MINTED Log:', JSON.stringify(log));
            ...
            tokenArr.push(xDtokenInfo);
            insertList.push(_checkAndInsertToken(xDtokenInfo, tokenDao))
          }
          break;
        case EventHashMap.DTOKEN_VOUCHER_BURNED:
          if (contractMap.DTokenTokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DTOKEN_VOUCHER_BURNED);
            Logger.log('Decoded DTOKEN_VOUCHER_BURNED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let xDtokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DTOKEN_TOKEN_LOCKED:
          if (contractMap.DTokenTokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DTOKEN_TOKEN_LOCKED);
            Logger.log('Decoded DTOKEN_TOKEN_LOCKED Log:', JSON.stringify(log));
            let xDtokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: get(log, 'decode.result[1]').toLowerCase(),
          ...
          break;
        case EventHashMap.DTOKEN_TOKEN_UNLOCKED:
          if (contractMap.DTokenTokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DTOKEN_TOKEN_UNLOCKED);
            Logger.log('Decoded DTOKEN_TOKEN_UNLOCKED Log:', JSON.stringify(log));
            let xDtokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: xChainName,
          ...
          break;
        case EventHashMap.DNC20_VOUCHER_MINTED:
          if (contractMap.DNC20TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC20_VOUCHER_MINTED);
            Logger.log('Decoded DNC20_VOUCHER_MINTED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DNC20_VOUCHER_BURNED:
          if (contractMap.DNC20TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC20_VOUCHER_BURNED);
            Logger.log('Decoded DNC20_VOUCHER_BURNED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DNC20_TOKEN_LOCKED:
          if (contractMap.DNC20TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC20_TOKEN_LOCKED);
            Logger.log('Decoded DNC20_TOKEN_LOCKED Log:', JSON.stringify(log));
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: get(log, 'decode.result[1]').toLowerCase(),
          ...
          break;
        case EventHashMap.DNC20_TOKEN_UNLOCKED:
          if (contractMap.DNC20TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC20_TOKEN_UNLOCKED);
            Logger.log('Decoded DNC20_TOKEN_UNLOCKED Log:', JSON.stringify(log));
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: xChainName,
          ...
          break;
        case EventHashMap.DNC721_VOUCHER_MINTED:
          if (contractMap.DNC721TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC721_VOUCHER_MINTED);
            Logger.log('Decoded DNC721_VOUCHER_MINTED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DNC721_VOUCHER_BURNED:
          if (contractMap.DNC721TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC721_VOUCHER_BURNED);
            Logger.log('Decoded DNC721_VOUCHER_BURNED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DNC721_TOKEN_LOCKED:
          if (contractMap.DNC721TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC721_TOKEN_LOCKED);
            Logger.log('Decoded DNC721_TOKEN_LOCKED Log:', JSON.stringify(log));
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: get(log, 'decode.result[1]').toLowerCase(),
          ...
          break;
        case EventHashMap.DNC721_TOKEN_UNLOCKED:
          if (contractMap.DNC721TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC721_TOKEN_UNLOCKED);
            Logger.log('Decoded DNC721_TOKEN_UNLOCKED Log:', JSON.stringify(log));
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: xChainName,
          ...
          break;
        case EventHashMap.DNC1155_VOUCHER_MINTED:
          if (contractMap.DNC1155TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC1155_VOUCHER_MINTED);
            Logger.log('Decoded DNC1155_VOUCHER_MINTED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DNC1155_VOUCHER_BURNED:
          if (contractMap.DNC1155TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC1155_VOUCHER_BURNED);
            Logger.log('Decoded DNC1155_VOUCHER_BURNED Log:', JSON.stringify(log));
            const chainId = get(log, 'decode.result[0]').split('/')[0];
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
          ...
          break;
        case EventHashMap.DNC1155_TOKEN_LOCKED:
          if (contractMap.DNC1155TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC1155_TOKEN_LOCKED);
            Logger.log('Decoded DNC1155_TOKEN_LOCKED Log:', JSON.stringify(log));
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: get(log, 'decode.result[1]').toLowerCase(),
          ...
          break;
        case EventHashMap.DNC1155_TOKEN_UNLOCKED:
          if (contractMap.DNC1155TokenBank.indexOf(get(log, 'address')) === -1) break;
          if (typeof get(log, 'decode') !== "object") {
            log = decodeLogByAbiHash(log, EventHashMap.DNC1155_TOKEN_UNLOCKED);
            Logger.log('Decoded DNC1155_TOKEN_UNLOCKED Log:', JSON.stringify(log));
            let tokenInfo = {
              _id: tx.hash.toLowerCase() + i,
              hash: tx.hash.toLowerCase(),
              from: xChainName,
...
function _getContractAddressSetByTxs(txs) {
  ...
  for (let tx of txs) {
    ...
    logs.forEach(log => {
      if (get(log, 'topics[0]') === EventHashMap.TRANSFER
        || get(log, 'topics[0]') === EventHashMap.DTOKEN_VOUCHER_MINTED || get(log, 'topics[0]') === EventHashMap.DTOKEN_VOUCHER_BURNED
        || get(log, 'topics[0]') === EventHashMap.DTOKEN_TOKEN_LOCKED || get(log, 'topics[0]') === EventHashMap.DTOKEN_TOKEN_UNLOCKED
        || get(log, 'topics[0]') === EventHashMap.DNC20_VOUCHER_MINTED || get(log, 'topics[0]') === EventHashMap.DNC20_VOUCHER_BURNED
        || get(log, 'topics[0]') === EventHashMap.DNC20_TOKEN_LOCKED || get(log, 'topics[0]') === EventHashMap.DNC20_TOKEN_UNLOCKED
        || get(log, 'topics[0]') === EventHashMap.DNC721_VOUCHER_MINTED || get(log, 'topics[0]') === EventHashMap.DNC721_VOUCHER_BURNED
        || get(log, 'topics[0]') === EventHashMap.DNC721_TOKEN_LOCKED || get(log, 'topics[0]') === EventHashMap.DNC721_TOKEN_UNLOCKED
        || get(log, 'topics[0]') === EventHashMap.DNC1155_VOUCHER_MINTED || get(log, 'topics[0]') === EventHashMap.DNC1155_VOUCHER_BURNED
        || get(log, 'topics[0]') === EventHashMap.DNC1155_TOKEN_LOCKED || get(log, 'topics[0]') === EventHashMap.DNC1155_TOKEN_UNLOCKED) {
        const address = get(log, 'address');
        if (address !== undefined && address !== ZeroAddress) {
          set.add(get(log, 'address'))
        }
